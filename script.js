class GameBoard {
  ...
  generateBoard(size) {
    ...
    let vowelCount = Math.floor(size * size * 0.3);
    let consonantCount = size * size - vowelCount;
    const board = [];
    for (let i = 0; i < size; i++) {
      const row = [];
      for (let j = 0; j < size; j++) {
        if (vowelCount > 0 && Math.random() < 0.3) {
...
// update CSS variable for letter size based on board size
document.documentElement.style.setProperty('--letter-size', `calc(80vw / ${boardSize})`);
